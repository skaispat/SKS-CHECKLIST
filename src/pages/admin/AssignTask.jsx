...
export default function AssignTask() {
  ...
  const fetchMasterSheetOptions = async () => {
    try {
      ...
      // Process all rows starting from index 1 (skip row 1, start from row 2)
      data.table.rows.forEach((row, index) => {
        if (index === 0) return; // Skip the first row (row 1)
  ...
  const fetchWorkingDays = async () => {
    try {
      ...
      const sheetName = "Working Day Calendar"; // Fixed the sheet name
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await fetch(
        "https://script.google.com/macros/s/AKfycbxVfzNQ08ZD2r9wEm3qRlfBX2hxVfE2AoM53pVlAS3PpgBcrtzNukhjdcdvcGxI13sx/exec",
        {
          method: "POST",
          body: formPayload,
          mode: "no-cors",
